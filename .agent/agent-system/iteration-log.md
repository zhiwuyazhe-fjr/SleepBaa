...
目标：让普通 Agent 执行结果也能反哺长期记忆和自我进化，不再只依赖 prompt 级 `memory.upsert` 或晨间反馈。

计划：

- [x] 扩展 `AgentToolCallDoc`，记录工具调用是否真正 `committed`，让审计记录能区分成功返回和已提交业务动作。
- [x] 新增 Agent 执行结果记忆合成逻辑，把成功提交、跳过和失败工具调用转成 `agent_action` / `strategy_weight` 记忆。
- [x] 对宿舍协同、capture、音频等可映射行动写入弱策略权重，晨间反馈仍作为更高可信的效果证据。
- [x] 撤销动作成功后写入 undo 记忆，并对被用户撤销的行动策略降权。
- [x] 运行时在合成记忆后发出 `memory_updated`，并把新增记忆计入 `agent_runs.memorySyncedCount`。
- [x] 增加后端单测覆盖执行结果记忆、撤销记忆和真实 noisy dorm run 的记忆落库。
- [x] 跑 `npm --prefix functions run build`、`npm --prefix functions test`。

结果：

- Agent 自主执行链路现在会自动形成“做了什么、哪些没做成、哪些被撤销”的长期证据，后续规划可以读取这些弱信号。
- `strategy_weight` 中的执行结果只作为弱证据，避免把“工具成功执行”误当成“用户睡眠改善”；晨间反馈仍负责更强的有效/无效判断。
- 撤销不再只是审计字段变化，也会进入自我进化闭环，帮助后续减少重复触发用户刚撤回的动作。
